...
def cross_product_sign(p1, p2):
    if p2[1] != 0:
        sign = -np.sign(p1[0] - p1[1] * (p2[0] / p2[1]))
    elif p2[0] != 0:
        sign = np.sign(p1[1]) * np.sign(p2[0])
    else:
        sign = 0
...
def graham(N, points, p0_index):
    p0 = np.array(points.pop(p0_index), dtype=data_type())
    norm_ps = np.array(points, dtype=data_type()) - p0
    norm_ps = np.array(sorted(norm_ps, key=functools.cmp_to_key(angle_comparator)))
    hull = np.zeros((N, 2), dtype=data_type())
    hull[0, :] = np.zeros(2)
    start_i = 0
...
def main():
    ...
    ordered_hull = np.concatenate(
        (hull[right_index:], hull[:right_index]), dtype=data_type()
    )
    toc = time.perf_counter()
    print(f"    Order time {toc - tic}", file=sys.stderr)
    print(m)
    if is_integer:
        ordered_hull = ordered_hull.astype("int")
        for point in ordered_hull[:, :]:
